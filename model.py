...
class Model():
    ...
    def FusionModel(self, iris_model, iris_upper_model, iris_lower_model):

        iris_model = tf.keras.models.Model(inputs=iris_model.input,
                                           outputs=iris_model.get_layer('fusion_layer').output)

        iris_upper_model = tf.keras.models.Model(inputs=iris_upper_model.input,
                                                 outputs=iris_upper_model.get_layer('fusion_layer').output)

        iris_lower_model = tf.keras.models.Model(inputs=iris_lower_model.input,
                                                 outputs=iris_lower_model.get_layer('fusion_layer').output)
        ...
        fusion = tf.keras.layers.Concatenate(name='concat_layer')([encoded_iris, encoded_upper, encoded_lower])

        prediction = tf.keras.layers.Dense(2)(fusion)
